...
int findActiveSignals(ActiveSignal *signal, const char *target_device, const char *target_snmpTrapOid, json_t *target_entities)
{
    ...
    int snmpTrapOid_found = 0;
    for (int i = 0; i < signal->snmpTrapOids_count; i++)
    {
        if (strcmp(signal->snmpTrapOids[i], target_snmpTrapOid) == 0)
        {
            snmpTrapOid_found = 1;
            break;
        }
...
void createSignal(StatefulRule *rule, const char *device, const char *snmpTrapOid, json_t *tags, const char *eventIdStr)
{
    if (!rule)
    {
