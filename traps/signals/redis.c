...
int load_active_signals_from_redis(redisContext *c) {
    ...
    for (size_t i = 0; i < reply->elements && active_signal_count < MAX_ACTIVE_SIGNALS; ++i) {
        ...
        if (json_is_array(events_array)) {
            sig->event_count = json_array_size(events_array);
            for (int j = 0; j < sig->event_count && j < 100; ++j) {
                const char *val = json_string_value(json_array_get(events_array, j));
                if (val) {
                    strncpy(sig->events[j], val, sizeof(sig->events[j]));
                }
            }
        }
...
int store_signal_in_redis(redisContext *ctx, const ActiveSignal *signal) {
    ...
    json_object_set_new(json, "events", events_array);
