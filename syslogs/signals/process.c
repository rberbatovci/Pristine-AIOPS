...
void process_message(rd_kafka_t *rk)
{
    ...
    while (1)
    {
        ...
        json_t *mnemonic_field = json_object_get(root, "mnemonic");
        ...
        const char *mnemonic = json_string_value(mnemonic_field);
        ...
        {
            if (findActiveSignals(&active_signals[i], device, mnemonic, tags))
            {
        ...
        RuleMatch *matches = findSignalRule(mnemonic, tags, &rule_match_count);
        ...
        if (rule_match_count > 0)
        {
            for (int i = 0; i < rule_match_count; i++)
            {
                ...
                if (type == MATCH_OPEN)
                {
                    ...
                    {
                        createSignal(rule, device, mnemonic, tags, eventIdStr, timestamp);
                    }
                }
