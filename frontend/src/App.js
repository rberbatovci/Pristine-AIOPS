...
import FilterTraffic from './components/netflow/Filters.js';
...
const App = ({ keycloak, keycloakAuthenticated }) => {
  ...
  const [searchEvent, setSearchEvent] = useState({ type: null, value: '' });
  ...
  const handleSearchChange = ({ type, value }) => {
    ...
    console.log(`Search event: type=${type}, value=${value}`);
  };
  ...
  const options = useMemo(
    () => (isDarkTheme ? darkThemeOptions : lightThemeOptions),
    [isDarkTheme]
  );
  ...
  const renderPopup = () => {
    ...
    switch (activePopup) {
      ...
      case "filter-traffic":
        return (
          <div className="menuOption" style={{ width: '480px' }}>
            <FilterTraffic keycloak={keycloak} onSelectedTrafficFiltersChange={handleFiltersChange} />
          </div>
        );
      case "syslog-event-table-tags":
        return (
          <div className="menuOption" style={{ width: '230px' }}>
      ...
      case "user-profile":
        return (
          <div className="userProfile" >
            <UserProfile keycloak={keycloak} currentUser={currentUser} setCurrentUser={setCurrentUser} setIsDarkTheme={setIsDarkTheme} onLogout={handleLogout} isDarkTheme={isDarkTheme} />
          </div>
        );
      default:
        return null;
  ...
  return (
    <BrowserRouter>
      {!isAuthenticated ? (
        <Routes>
          <Route path="/login" element={<Login keycloak={keycloak} isDarkTheme={isDarkTheme} onGuestLogin={handleGuestLogin} />} />
        ...
        <div data-theme={isDarkTheme ? "dark" : "light"} className="App">
          ...
          <div className="header">
            <Header currentUser={currentUser} dashboardTitle={dashboardTitle} showNotification={showNotification} onTogglePopup={togglePopup} selectedDevice={selectedDevice} onSearchChange={handleSearchChange} />
          </div>
          {activePopup && (
