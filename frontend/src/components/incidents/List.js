...
function List({ onIncidentSelect, signals }) {
  ...
  const handleSignalClick = (signal) => {
    ...
    onIncidentSelect(signal); // Pass the selected signal's ID to the parent
  };
    ...
    <div className="signals-list-container">
      <ul className="signals-list" style={{ paddingLeft: '0', listStyle: 'none' }}>
        {signals.map((signal) => (
          ...
          >
            <div className="signal-item-content" style={{ display: 'flex' }}>
              {/* Signal Badge */}
              <div className="signalBadge">
                <div
                  style={{
                    background: getAlertColor(signal.state),
                    height: '95%',
                    width: '20px',
                    position: 'relative',
                    top: '-2px',
              ...
              {/* Signal ID and State */}
              <div className="signalIDnState" style={{ paddingLeft: '30px' }}>
                <div><strong className="strongText">ID: </strong><span className="spanText">#{signal.id}</span></div>
                <div><span className="spanText">{signal.state}</span></div>
                <div><span className="spanText">{signal.signalSource}</span></div>
              </div>

              {/* Signal Details */}
              <div className="signalDetails" style={{ paddingLeft: '30px' }}>
                ...
                <div style={{ marginTop: '5px', fontSize: '12px', display: 'flex' }}>
                  {signal.affectedEntity && Object.keys(signal.affectedEntity).length > 0 ? (
                    Object.keys(signal.affectedEntity).map((key) => (
                      <div key={key} style={{ marginRight: '10px' }}>
                        <strong className="strongText">{key}:</strong> <span className="spanText">{signal.affectedEntity[key]}</span>
                      </div>
                    ))
                  ) : (
