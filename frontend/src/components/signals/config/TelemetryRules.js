...
import { useTelemetryRules } from "../../../hooks/useTelemetryRules";

const TelemetryRules = ({ keycloak, devices, onReload, tags, showNotification }) => {
    const [snmpTrapRule, setSnmpTrapRule] = useState([]);
    const { rules, ruleDetails, loading: isLoading, error, selectRule, addRule, updateRule, deleteRule } = useTelemetryRules(keycloak);
    const emptyRule = {
        name: '',
        devices: [],
        highthreshold: '',
