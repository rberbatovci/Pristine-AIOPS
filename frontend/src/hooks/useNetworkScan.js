...
export default function useNetworkScan(keycloak) {
      ...
      const interval = setInterval(async () => {
        try {
          ...
          if (response.status === "completed") {
            ...
            clearInterval(interval);
            reject(new Error(response.error || "Network sweep task failed."));
          }
        } catch (err) {
          clearInterval(interval);
  ...
  const scanNetwork = async (cidrRange) => {
    ...
    setDevices([]);
    try {
      const initResponse = await kcFetch(keycloak, "/scans/network-sweep", {
        method: "POST",
        body: JSON.stringify({ target_range: cidrRange }),
      });
  ...
  return { loading, devices, setDevices, error, setError, scanNetwork };
}