...
func (s *grpcServer) MdtDialout(stream dialout.GRPCMdtDialout_MdtDialoutServer) error {
	for {
		...
		// Extract subscription semsor path
		sensorPath := telemetryMsg.EncodingPath
		if sensorPath == "" {
			sensorPath = "unknown"
		}
		...
		log.Printf("📥 Received telemetry data from device: %s, subscription ID: %s", nodeId, sensorPath)

		// Send data to Kafka
		go sendToKafkaTopic(sensorPath, in.Data)
...
func sendToKafkaTopic(sensorPath string, data []byte) {
	// Lookup topic
	topic := "telemetry.unknown"
	for key, mappedTopic := range subscriptionTopicMap {
		if containsIgnoreCase(sensorPath, key) {
			topic = mappedTopic
			break
		}
	}
